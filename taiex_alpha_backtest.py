...
# 成本 = 單邊 |Δpos| × 價格 × 0.0002 以權益近似（實作成 |Δpos| × COST_RATE × equity）
# --rebuild：只讀 BestConfig.json，絕不改動參數；輸出 PPT：
# 1 KPI → 2 Equity → 3 六宮格（左下為 Long/Flat/Short 線圖）→ 4 WF OOS
...
COST_RATE    = 0.0002      # 單邊成本
TRADING_DAYS = 252
MOM_N        = 10          # 10日動能（固定，不動策略）
    ...
    with zipfile.ZipFile(TEACHER_ZIP, "r") as zf:
        ...
        if fea is not None:
            try:
                ...
                raise ImportError("ZIP 內是 feather，但環境缺 pyarrow；請安裝 pyarrow 或使用含 csv 的資料包")
...
def up_down_capture(strat_ret: pd.Series, bench_ret: pd.Series):
    ...
    m = df.resample("ME").sum()  # month-end，避免 'M' deprecation
    up = m[m["b"]>0]; dn = m[m["b"]<0]
    up_cap = np.nan if up["b"].sum()==0 else up["s"].sum()/up["b"].sum()
    dn_cap = np.nan if dn["b"].sum()==0 else up["s"].sum()/dn["b"].sum()
...
# ========= 策略：10日動能做多 + 波動目標槓桿 + 濾網(off/soft) =========
# 必要欄位：vol_lb, vol_target, lev_cap, rebalance_days, smooth_days, trend_filter_mode, ma_long
...
# ========= Walk-Forward（僅供 --rebuild 時做 OOS 表） =========
def wf_splits(df: pd.DataFrame, train_m=24, test_m=6, step_m=6, max_splits=10):
    sidx = df.index
...
def plot_panel(daily_df: pd.DataFrame, track: pd.DataFrame, kpi: dict,
    ...
    ax3.set_title("Comparable")
        ...
        # 4) Long / Flat / Short 「累積進出次數」線圖（非水平）
...
# ========= 主流程（僅 rebuild） =========
def main():
    assert "--rebuild" in sys.argv, "本版僅提供 --rebuild（鎖定 BestConfig.json）模式"
    df = read_teacher_bundle()
    sample_info = f"樣本期間：{df.index.min().date()} ~ {df.index.max().date()}  天數：{len(df):,}"

    # 只讀 BestConfig.json（不補欄位，不正規化）
    assert os.path.exists("BestConfig.json"), "找不到 BestConfig.json（請放在同目錄）"
    with open("BestConfig.json","r",encoding="utf-8") as f:
        best_cfg = json.load(f)
    for k in REQUIRED_KEYS:
        assert k in best_cfg, f"BestConfig.json 缺少鍵：{k}"

    # 先做 WF OOS 表（同一組參數）
    wf_rows = wf_for_one(df, best_cfg)
    ...
    make_ppt(best_cfg, best_kpi, eq_png, panel_png, wf_rows, pptx, sample_info)

    # 終端摘要
    print("[DONE][REBUILD] 輸出：", pptx, eq_png, panel_png, flush=True)
    ...
    print("[最佳參數]：", json.dumps(best_cfg, ensure_ascii=False), flush=True)
